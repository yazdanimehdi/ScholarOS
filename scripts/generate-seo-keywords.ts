...
const STOP_WORDS = new Set([
  ...
  "because", "before", "between", "during", "here", "how", "if",
  "into", "its", "it", "me", "my", "off", "once", "only", "our",
  "out", "over", "own", "same", "she", "he", "her", "his", "him",
  ...
  "new", "one", "two", "using", "based", "via", "etc", "use", "used",
  "show", "shows", "work", "works", "approach", "method", "results",
  "paper", "study", "present", "propose", "proposed", "novel",
  "first", "well", "also", "however", "thus", "therefore",
]);
...
function main() {
  ...
  if (fs.existsSync(RESEARCH_CONFIG_PATH)) {
    const research = yaml.load(
      fs.readFileSync(RESEARCH_CONFIG_PATH, "utf-8"),
  ...
  if (fs.existsSync(SITE_CONFIG_PATH)) {
    const site = yaml.load(
      fs.readFileSync(SITE_CONFIG_PATH, "utf-8"),
    ) as Record<string, unknown>;
    if (site?.description) {
  ...
  const updated = siteContent.replace(
    /^(\s*keywords:\s*)"[^"]*"/m,
    `$1"${keywordsValue}"`,
  );
  ...
  if (updated === siteContent) {
    ...
    const updated2 = siteContent.replace(
      /^(\s*keywords:\s*).*$/m,
      `$1"${keywordsValue}"`,
    );
